...
computed (same heuristics as posture_check.py's forward-head check and
phone_detection/monitor.py's object-proximity check, just ported to run
here against MoveNet/EfficientDet output instead of MediaPipe), plus each
model's inference time and server-side receive/decode time. The client
...
# --- posture heuristic (ported from ../posture_check.py: forward-head via
# ear-vs-shoulder x-offset, normalized by shoulder width) ---
NOSE, LEFT_EYE, RIGHT_EYE, LEFT_EAR, RIGHT_EAR = 0, 1, 2, 3, 4
LEFT_SHOULDER, RIGHT_SHOULDER = 5, 6
POSE_CONF_THRESHOLD = 0.3
FORWARD_HEAD_THRESHOLD = 0.35

# --- phone/drink proximity heuristic (ported from
# ../phone_detection/monitor.py's active_proximity_states) ---
SCORE_THRESHOLD = 0.4
