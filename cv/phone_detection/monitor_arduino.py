...
Copy of monitor.py's job (posture + phone/drink -> POST /api/robot/sensors)
but with all CV inference moved off the Mac and onto the Arduino Uno Q: this
script just grabs/loads a frame, ships it over TCP to
bench/tcp_infer_server.py running on the device (MoveNet Lightning +
EfficientDet-Lite0 + the int8 Edge Impulse classifier, all running on-device,
heuristics computed on-device too -- see that file), and POSTs whatever
{doomscrolling, slouching, sleeping, drinkingWater, tempRaw} reading comes
back. No MediaPipe/TFLite dependency needed on the Mac side at all.

For now this runs a single shot against a static image (e.g. a photo taken
from the Mac's own webcam) rather than a continuous camera loop -- see
monitor.py for the original 1Hz live-camera version.

Usage:
    python3 monitor_arduino.py --arduino-host 10.31.181.91 --image ../new-models/mac_test_shots/mac_webcam.jpg
